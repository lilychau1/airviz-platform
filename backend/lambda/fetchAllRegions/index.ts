...
// Utility: check if timestamp is "on the hour"
...
export const handler = async (event: APIGatewayProxyEventV2) => {
    ...
    try {
        ...
        if (typeof dbCreds === "string") throw new Error("Expected DB secret to be a JSON object");
        ...
        const records: RegionRecord[] = latestRes.rows.map((row: RegionRow) => ({
            ...
            currentAqiColour: row.colour_code ? (row.colour_code as Colour) : null,
        }));
