...
const handler = async () => {
    ...
    try {
        ...
        const s3Resp = await s3Client.send(new client_s3_1.GetObjectCommand({ Bucket: bucket, Key: key }));
        const jsonString = await streamToString(s3Resp.Body);
        const boroughs = JSON.parse(jsonString);
        ...
        if (boroughs.length > 0) {
            ...
            const valuesClause = boroughs
                .map((_, i) => `($${i * 7 + 1}, $${i * 7 + 2}, POINT($${i * 7 + 3}, $${i * 7 + 4}), $${i * 7 + 5}, $${i * 7 + 6}, $${i * 7 + 7})`)
                .join(', ');
            // Flatten parameters for all of the boråoughs
            const params = boroughs.flatMap((borough, i) => [
                i + 1, // id
                borough.name, // name
            ...
            // Run query
            await client.query(`INSERT INTO boroughs (id, name, location, inserted_at, updated_at, description) VALUES ${valuesClause}`, params);
        ...
        const tileResp = await s3Client.send(new client_s3_1.GetObjectCommand({ Bucket: bucket, Key: tileKey }));
        const csvString = await streamToString(tileResp.Body);
        ...
        const tiles = tileLines.map(line => {
        ...
        if (tiles.length > 0) {
            const valuesClause = tiles
                .map((tile, i) => `($${i * 10 + 1}, $${i * 10 + 2}, $${i * 10 + 3}, $${i * 10 + 4}, $${i * 10 + 5}, POINT($${i * 10 + 6}, $${i * 10 + 7}), $${i * 10 + 8}, $${i * 10 + 9}, $${i * 10 + 10})`)
                .join(', ');
            ...
            await client.query(`INSERT INTO tiles (id, borough_id, zone_id, postcode_area_id, name, location, inserted_at, updated_at, description) VALUES ${valuesClause}`, params);
