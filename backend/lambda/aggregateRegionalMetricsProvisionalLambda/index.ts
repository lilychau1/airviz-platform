...
export const handler = async (event: APIGatewayProxyEventV2) => {
    ...
    try {
        ...
        const stateRes = await client.query(
            `SELECT last_processed_timestamp FROM aggregate_state_provisioned_lambda WHERE level = $1 FOR UPDATE`,
            [level]
        );
        ...
        if (stateRes.rows.length === 0) {
            await client.query(
                `INSERT INTO aggregate_state_provisioned_lambda(level, last_processed_timestamp) VALUES ($1, $2)`,
                [level, lastProcessed]
            );
        }
        ...
        const last30dRes = await client.query(`
            SELECT
                ...
                aqi.value,
                ar.timestamp
            FROM tiles_provisioned_lambda t
            JOIN aq_records_provisioned_lambda ar ON t.id = ar.tile_id
            JOIN air_quality_index_provisioned_lambda aqi ON aqi.record_id = ar.id
            WHERE ar.timestamp >= $1
        ...
        for (const agg of finalAggregates) {
            ...
            await client.query(`
                INSERT INTO regional_aggregates_provisioned_lambda(
        ...
        if (finalAggregates.length > 0) {
            ...
            await client.query(
                `UPDATE aggregate_state_provisioned_lambda SET last_processed_timestamp=$1 WHERE level=$2`,
